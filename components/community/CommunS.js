import React from 'react'
import DrawerMenu from '../../container/sidebar'
import Searchd from '../Mobile/Home/searchd'
import CarouselWithCustomDots from '../Home/Advertisement'
import NavBar from '../Home/NavBar'
import KosbisBG from '../../assets/kosbis1.png'
import Image from 'next/image'
import lx from './Rectangle907.png'
import jk from '../community/Ellipse68.png'
import jl from '../community/Ellipse69.png'
import jh from '../community/Ellipse70.png'
import js from '../community/Ellipse71.png'
import { BsThreeDotsVertical } from 'react-icons/bs'
import Post from '../Post'
import Footer from '../Mobile/Home/footer'

const style = 
{
    wrapper: `flex-col h-full w-full bg-[#FFFFFF] border-[#38444d]  `,
    header: ` top-0  z-10 px-3 sticky bg-[#FFFFFF] flex justify-between items-center`,
    headerTitle: `text-xl font-bold`,
}

const Messages = [ 
    {
      displayName: 'Innovator_29',
      avatar: 'data:image/jpeg;base64,',
      text: 'Oi guys check out my new art #digiart #superartist',
      image: '',
      timestamp: '2022-05-25T12:00:00.000Z' //This is how sanity stores timestamp
    },
    
    {
      displayName: 'Innovator_29',
      avatar: 'data:image/jpeg;base64,',
      text: 'Everything would be alright at the end of the day, if you spot a problem then you are the solution',
      timestamp: '2021-05-25T12:00:00.000Z' //This is how sanity stores timestamp
    },

    {
      displayName: 'Innovator_29',
      avatar: 'data:image/jpeg;base64,',
      text: 'End ASUU Strike we want to graduate You know? People pikin suppose don graduate by now o',
      timestamp: '2021-03-10T12:00:00.000Z' //This is how sanity stores timestamp
    },

      {
          displayName: 'Innovator_29',
          avatar: 'data:image/jpeg;base64,',
          text: 'Lorem Ipsum I nor know the remaining part abeg make i check something',
          timestamp: '2020-07-12T12:00:00.000Z' //This is how sanity stores timestamp
      },
  ]

const CommunS = () => {
  return (
    <div className={style.wrapper}>
    <div className={style.header}>
     <div className='text-black font-bold w-full  pt-2 pb-3 flex  justify-between'>
     <DrawerMenu/>
     <Image src={KosbisBG} alt='kb' width={125} height={35} className='pt-1' />
    <Searchd/>
    
     </div>
</div>
<div className=' w-full'>
 <CarouselWithCustomDots/>
</div>
<div className='flex-[1] sticky border-b border-[#C4C4C4] justify-center'> <NavBar initialSelectedIcon={'World'} /></div>
<div className='w-full'>
    <Image src={lx} className='w-full' />
    <div className='bg-[#FF7F23] h-[132px] -mt-[6px]'>
        <div className='px-[35px] flex justify-between pt-[11px] text-white text-[16px] font-semibold'>Book Club <BsThreeDotsVertical/></div> 
        <div className='flex'>
        <div className='pt-[9px] px-[35px] pr-[15px]'>
        <Image src={jk} height={24} width={24} /> 
        <Image src={jl} height={24} width={24} /> 
        <Image src={jh} height={24} width={24} />
        <Image src={js} height={24} width={24} /> 
      </div>
        <div className='text-[8px] font-semibold text-white pt-4'>150 Members</div>
      </div>
      <div className='text-[12px] font-semibold px-[35px] text-white'>A safe Space for Everything Books</div>
      <div className='border-[1px] border-white w-[70px] text-center mt-[7px] text-white mx-[35px] px-[3px] rounded-b-[24px] rounded-t-[24px] h-[26px]'>Join</div>
    </div>
</div>
<div >
    {Messages.map((message, index)=>
    <Post 
        key={index}
        displayName={message.displayName}
        text={message.text}
        timestamp={message.timestamp}
        />
       
    )}
     
          <Footer/>

    </div>
    </div>
  )
}

export default CommunS